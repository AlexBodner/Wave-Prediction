...
def float_to_builtin_time(t: float) -> Time:
    secs = int(float(t))
    nanosecs = int((float(t) - secs) * 1e9)
    return Time(sec=secs, nanosec=nanosecs)

def process_bag(bag_path, output_folder, n_mean_plane=5):
    os.makedirs(output_folder, exist_ok=True)

    # --------- FIRST PASS: Collect n_mean_plane point clouds for mean plane ---------
    tf_buffer = tf2_ros.Buffer(cache_time=rclpy.time.Duration(seconds=500000000))
    camera_info_msg = None
    imu_T_imu_optical = TransformStamped()
    mean_plane_points = []
    mean_plane_timestamps = []
        ...
        for connection, timestamp, rawdata in reader.messages():
            ...
            if camera_info_msg and connection.topic == '/camera/camera/depth/image_rect_raw/compressedDepth':
                ...
                try:
                    ...
                    points_in_enu = (enu_T_depth_optical @ points_hom)[:3, :].T

                    mean_plane_points.append(points_in_enu)
                    mean_plane_timestamps.append(timestamp)
                except Exception as e:
                    print(f"Error processing compressedDepth message: {e}")
                    continue
            if len(mean_plane_points) >= n_mean_plane:
                break

    if not mean_plane_points:
        print("No points collected for mean plane.")
        return

    all_points_concat = np.concatenate(mean_plane_points, axis=0)
    centroid, R, normal, u, v = fit_mean_plane.compute_plane_transform_ransac(
        all_points_concat, thresh=0.01, max_iterations=100
    )
    ...
    # --------- SECOND PASS: Process all frames one by one ---------
    tf_buffer = tf2_ros.Buffer(cache_time=rclpy.time.Duration(seconds=500000000))
    camera_info_msg = None
    imu_T_imu_optical = TransformStamped()
        ...
        for connection, timestamp, rawdata in reader.messages():
            ...
            if camera_info_msg and connection.topic == '/camera/camera/depth/image_rect_raw/compressedDepth':
                try:
                    ...
                    grid = generate_weighted_height_grid(
                        points_in_enu, grid_size=(100, 100), origin=(-7, -6), max_coords=(3, 3), radius=0.2
                    )
